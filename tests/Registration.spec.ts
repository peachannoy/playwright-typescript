...
test(`Registration test number ${i}`, async ({ page }) => {
    ...
    const phone: number = faker.datatype.number({min: 1000000000, max: 9999999999});
    ...
    const bool = await registrationPage.register(firstName, lastName, email, phone, occupation, gender, password);
